...
describe("FileClient - OAuth", () => {
  ...
  beforeEach(async function (this: Context) {
    recorder = record(this, recorderEnvSetup);
    const serviceClient = getTokenBSU("", "", { fileRequestIntent: "backup" });
    shareName = recorder.getUniqueName("share");
    shareClient = serviceClient.getShareClient(shareName);
...
describe("FileClient - AllowTrailingDots - True", () => {
  ...
  beforeEach(async function (this: Context) {
    recorder = record(this, recorderEnvSetup);
    const serviceClient = getBSU({ allowSourceTrailingDot: true, allowTrailingDot: true });
    shareName = recorder.getUniqueName("share");
    shareClient = serviceClient.getShareClient(shareName);
    await shareClient.create();
  ...
  it("startCopyFromURL", async () => {
    await fileClient.create(1024);
    const copiedFileName = recorder.getUniqueName("copiedfile");
...
describe("FileClient - AllowTrailingDots - False", () => {
  ...
  beforeEach(async function (this: Context) {
    recorder = record(this, recorderEnvSetup);
    const serviceClient = getBSU({ allowSourceTrailingDot: false, allowTrailingDot: false });
    shareName = recorder.getUniqueName("share");
    shareClient = serviceClient.getShareClient(shareName);
    await shareClient.create();
  ...
  it("rename", async () => {
    await fileClient.create(1024);
    const destFileName = recorder.getUniqueName("destfile");
    const destFileNameWithTrailingDots = destFileName + "....";
    const result = await fileClient.rename(destFileNameWithTrailingDots);
    assert.ok(
      result.destinationFileClient.name === destFileNameWithTrailingDots,
      "Destination name should be expected"
    );
